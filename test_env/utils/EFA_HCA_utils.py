...
from utils.graph_utils import  get_adj, Graph_Analysis
# from selfregulation.utils.utils import get_behav_data, get_demographics, get_info
from utils.r_to_py_utils import get_attr, get_Rpsych, psychFA
...
class HCA_Analysis():
    ...
    def name_clusters(self, names, inp):
        ...
        if len(names)!=num_clusters:
            names = ['cluster'+str(i) for i in range(num_clusters)]
...
class Results(EFA_Analysis, HCA_Analysis):
    ...
    def __init__(self, 
        ...
        """
        assert (data is not None and data_no_impute is not None) or saved_obj_file is not None
